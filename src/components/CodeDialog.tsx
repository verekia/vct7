...
function CodeDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="panel-surface border-line flex max-h-[88vh] w-[960px] max-w-[92vw] flex-col border shadow-2xl">
        <header className="border-line flex items-center justify-between border-b px-4 py-2.5">
          <h2 className="text-text text-xs font-semibold tracking-[2px] uppercase">SVG code</h2>
          <div className="flex gap-1.5">
            <CopyButton code={code} />
            <button type="button" className="px-2 py-[2px] text-[11px]" onClick={onClose}>
              Close
            </button>
          </div>
        </header>
        <div className="bg-bg-0 min-h-0 flex-1 overflow-auto p-3">
          <pre
            className="font-mono text-[14px] leading-[1.6] whitespace-pre-wrap select-text"
...
function CopyButton({ code }: { code: string }) {
  ...
  return (
    <button type="button" className="px-2 py-[2px] text-[11px]" onClick={onCopy}>
      {copied ? 'Copied' : 'Copy'}
    </button>
  )
}
...
const NAME_RE = /^[\w:.-]+/
const WS_RE = /^\s+/
const CSS_TOKEN_RE = /(@[\w-]+)|(\.[\w-]+|#[\w-]+)|([\w-]+)(\s*:)|([{};:,])|("[^"]*"|'[^']*')|(\s+)|([^\s{};:,]+)/g
  ...
  while (i < code.length) {
    ...
    if (code[i] === '<') {
      ...
      while (i < code.length) {
        ...
        if (attrMatch) {
          push(attrMatch[0].startsWith('data-v7-') ? CLS.attrV7 : CLS.attr, attrMatch[0])
